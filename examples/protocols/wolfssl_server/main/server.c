...
#define WOLFSSL_EXAMPLE_TASK_NAME "wolfssl_example"
#define WOLFSSL_EXAMPLE_TASK_STACK_WORDS 10240
#define WOLFSSL_EXAMPLE_TASK_PRIORITY 8
/* The event group allows multiple bits for each event,
   but we only care about one event - are we connected
   to the AP with an IP? */
const static int CONNECTED_BIT = BIT0;

const static char *TAG = "wolfssl_server";
...
THREAD_RETURN WOLFSSL_THREAD server_test(void *args)
{
    ...
    ESP_LOGI(TAG, " Start server_test");
...
ESP_LOGI(TAG, " SSL_CTX_new");
    ...
    if (wolfSSL_CTX_load_static_memory(&ctx, method, memory, sizeof(memory), 0, 1) != WOLFSSL_SUCCESS)
...
ESP_LOGI(TAG, "Set User CallBack");
...
//#ifndef TEST_LOAD_BUFFER
        int val;
        if ((val = wolfSSL_CTX_use_certificate_chain_buffer_format(ctx, server_cert_der_2048,
                                sizeof_server_cert_der_2048, WOLFSSL_FILETYPE_ASN1)) != SSL_SUCCESS)
        {
            ESP_LOGI(TAG, "ERROR: failed to load (ret=%d,) please check the file.\n", val);
        }
...
ESP_LOGI(TAG, " PrivateKey ");
    ...
    {
        int val;
        if((val=wolfSSL_CTX_use_PrivateKey_buffer(ctx, server_key_der_2048, 
            sizeof_server_key_der_2048, WOLFSSL_FILETYPE_ASN1)) != WOLFSSL_SUCCESS)

            ESP_LOGI(TAG, "Failed to load key.(ret=%d)", val);
        // #ifndef TEST_LOAD_BUFFER
...
ESP_LOGI(TAG, " doCliCertCheck ");
#if !defined(NO_FILESYSTEM) && !defined(NO_CERTS)
    /* if not using PSK, verify peer with certs
       if using PSK Plus then verify peer certs except PSK suites */
    if (doCliCertCheck && (usePsk == 0 || usePskPlus) && useAnon == 0)
    {
        SSL_CTX_set_verify(ctx, WOLFSSL_VERIFY_PEER | (usePskPlus ? WOLFSSL_VERIFY_FAIL_EXCEPT_PSK : WOLFSSL_VERIFY_FAIL_IF_NO_PEER_CERT), 0);
...
ESP_LOGI(TAG, " waiting request from client... ");
    while (1)
    {
        /* allow resume option */
        ESP_LOGI(TAG, "resumeCount = %d..", resumeCount);
        if (resumeCount > 1)
        {
            if (dtlsUDP == 0)
            {
                ...
                ESP_LOGI(TAG, " clientfd(%d)... ", clientfd);
            }
            else
            {
                ESP_LOGI(TAG, " tcp_listen(,,%d,%d,%d)... ", useAnyAddr, dtlsUDP, dtlsSCTP);
        ...
        ESP_LOGI(TAG, "Before creating SSL");
...
#endif
         ESP_LOGI(TAG, "OK. Before creating SSL");
        if (doMcast)
        {
            ESP_LOGI(TAG, "doMcast");
#ifdef WOLFSSL_MULTICAST
...
#endif
         ESP_LOGI(TAG, "Start SSL_new");
        ssl = SSL_new(ctx);
        if (ssl == NULL)
            err_sys_ex(runWithErrors, "unable to get SSL");
         ESP_LOGI(TAG, "Finish SSL_new");
#ifdef OPENSSL_EXTRA
...
#ifdef WOLFSSL_SEND_HRR_COOKIE
        ESP_LOGI(TAG, "Start WOLFSSL_SEND_HRR_COOKIE");
        if (hrrCookie && wolfSSL_send_hrr_cookie(ssl, NULL, 0) != WOLFSSL_SUCCESS)
        {
            err_sys("unable to set use of cookie with HRR msg");
        }
        ESP_LOGI(TAG, "Finish WOLFSSL_SEND_HRR_COOKIE");
#endif

#if defined(WOLFSSL_STATIC_MEMORY) && defined(DEBUG_WOLFSSL)
        ESP_LOGI(TAG, "After creating SSL");
        fprintf(stderr, "After creating SSL\n");
        if (wolfSSL_CTX_is_static_memory(ctx, &mem_stats) != 1)
            err_sys_ex(runWithErrors, "ctx not using static memory");
        if (wolfSSL_PrintStats(&mem_stats) != 1) /* function in test.h */
            err_sys_ex(runWithErrors, "error printing out memory stats");
         ESP_LOGI(TAG, "Finish After creating SSL");
#endif
...
ESP_LOGI(TAG, " DONE(waiting request from client...) ");
#ifndef NO_HANDSHAKE_DONE_CB
        ...
        if (!disableCRL)
        {
            ...
            if (wolfSSL_LoadCRL(ssl, crlPemDir, WOLFSSL_FILETYPE_PEM, crlFlags) != WOLFSSL_SUCCESS)
        ...
        if (postHandAuth)
        {
            SSL_set_verify(ssl, WOLFSSL_VERIFY_PEER | ((usePskPlus) ? WOLFSSL_VERIFY_FAIL_EXCEPT_PSK : WOLFSSL_VERIFY_FAIL_IF_NO_PEER_CERT), 0);
            if (SSL_CTX_load_verify_locations(ctx, verifyCert, 0) != WOLFSSL_SUCCESS)
            {
...
//int wolfssl_example_task(int argc, char **argv)
static void wolfssl_example_task(void *p)
{
    ESP_LOGI(TAG, " Start wolfssl_example_task");
...
static void wolfssl_example_client_init(void)
{
    int ret;
    xTaskHandle _handle;

    ret = xTaskCreate(wolfssl_example_task,
                      WOLFSSL_EXAMPLE_TASK_NAME,
                      WOLFSSL_EXAMPLE_TASK_STACK_WORDS,
                      NULL,
                      WOLFSSL_EXAMPLE_TASK_PRIORITY,
                      &_handle);

    if (ret != pdPASS)
    {
        ESP_LOGI(TAG, "create thread %s failed", WOLFSSL_EXAMPLE_TASK_NAME);
    }
}
...
#define EXAMPLE_WIFI_SSID "XXXX iPhone" /* */
#define EXAMPLE_WIFI_PASS "zzzzzyyyy"   /* */
